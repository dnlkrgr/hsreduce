...
data Poly (a :: Type) (b :: Type) = PCons (Poly b a)
...
hm :: (String -> Const (Endo [String]) String) -> Poly Int String -> Const (Endo [String]) (Poly Int String)
hm = param @0

class HasParam (p :: Nat) s a | p s a -> s, p s -> a where
  param :: Applicative g => (a -> g a) -> s -> g s

instance ( x ~ ArgAt (Poly a b) n , GHasParam n (RepN (Poly a b)) x) => HasParam n (Poly a b) x where
...
newtype Curried f a = Curried (forall r. f (a -> r) -> f r)
...
instance {-# OVERLAPPABLE #-} ( GHasParamRec (LookupParam si p) s a) => GHasParam p (Rec si s) a where
