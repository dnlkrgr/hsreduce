...
newtype Curried f a = Curried {w :: forall r. f (a -> r) -> f r}
...
instance (GHasParam p h h' a b, GHasParam p r r' a b) =>
         GHasParam p (h :*: r) (h' :*: r') a b where
...
instance GHasParam p s t a b =>
         GHasParam p (M1 m aj s) (M1 m aj t) a b where
  ab f (M1 x) = M1 <$> ab @p f x
instance GHasParamRec (LookupParam ak p) s t a b =>
...
type family ReplaceArg t an ao where
...
type family Zip a b where
  Zip (w mt m s) (w mt m t) = w mt m (Zip s t)
  Zip (l r) (l' :+: r') = l' :+: Zip r r'
  Zip (l :*: r) (l' :*: r') = Zip l l' :*: Zip r r'
  Zip (aq p) (aq a) = Rec p a
