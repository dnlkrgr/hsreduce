...
hsreduce isProject test filePath = do
  ...
  files <-
    if isProject
      then recListDirectory sourceDir
...
largestFixpoint f =
  ...
  where
    iterateFrom = do
      liftIO $ putStrLn "\n***NEW ITERATION***"
      oldOrmolu <- _parsed <$> get
      f
      newOrmolu <- _parsed <$> get
      let oldLength = BS.length . TE.encodeUtf8 . T.pack $ showGhc oldOrmolu
          newLength = BS.length . TE.encodeUtf8 . T.pack $ showGhc newOrmolu
